...
class testing_api2(APIView):
    def get(self, request):
        ...
        try:
            ...
            bulk_notifications(
                [
                    "dTjOZW_WQuOUUVjTwIGqsZ:APA91bEEUsNeQR_SJzNuCRbSYNTdaS9T0oiFsuG-FqRv8KRJUMkzFrqGJRpUkPnPrVuArJsheLkh1bSYeiKyLsgd5FTAKIC6P9DX0-AELFHGryrQ88R-_Ea27WPcacEyCZaXWg5RlpMl"
                ],
                "efsd",
                "efsdg",
            )
...
class SendNotifications(APIView):
    ...
    def post(self, request):
        ...
        if data["notiff_sett"] == 1:
            filter_notifications = api_models.FilterNotifications.objects.filter(
                lst_buy=True, domain=user.domain
            )
        elif data["notiff_sett"] == 2:
            filter_notifications = api_models.FilterNotifications.objects.filter(
                posts=True, domain=user.domain
            )
        elif data["notiff_sett"] == 3:
            filter_notifications = api_models.FilterNotifications.objects.filter(
                posts_admin=True, domain=user.domain
            )
        elif data["notiff_sett"] == 4:
            filter_notifications = api_models.FilterNotifications.objects.filter(
                events=True, domain=user.domain
            )
        elif data["notiff_sett"] == 5:
            filter_notifications = api_models.FilterNotifications.objects.filter(
                threads=True, domain=user.domain
            )
        elif data["notiff_sett"] == 6:
            filter_notifications = api_models.FilterNotifications.objects.filter(
                comments=True, domain=user.domain
            )
    ...
    def put(self, request):
        ...
        for i in users:
            if (
                data["notif_year"][i.year - 1] == "1"
                and (i.branch in data["notif_branchs"])
                and (i.course in data["notif_courses"])
            ):
                i.notif_seen = False
                i.notif_count += 1
                i.save()
                try:
                    ...
                    filter_notif = api_models.FilterNotifications.objects.get(
                        username=i
                    )
                    ...
                    a = 0
        bulk_notifications(
            fcm_tokens, user.email, "Gave Announcement : " + " : " + data["description"]
        )
        return Response({"error": False, "tokens": len(fcm_tokens)})
    ...
    def put(self, request):
        ...
        try:
            ...
            if old_team_mem != data["team_members"]:
                team_members_transfer(
                    old_team_mem, data["team_members"], data["id"], "club", data["name"]
                )
    ...
    def put(self, request):
        ...
        try:
            ...
            if data["image2_type"] == "file":
                new_sport.sport_ground_img = ContentFile(
                    base64.b64decode(data["file1"]), data["file_name1"]
                )
            new_sport.save()
            if old_team_mem != data["team_members"]:
                team_members_transfer(
...
class ALLFESTS_list(APIView):
    ...
    def put(self, request):
        ...
        try:
            ...
            if old_team_mem != data["team_members"]:
                team_members_transfer(
                    old_team_mem, data["team_members"], data["id"], "fest", data["name"]
                )
...
class ALL_BRANCHES(APIView):
    ...
    def get(self, request):
        ...
        try:
            user = request.user
            data = request.query_params
            all_branches = api_models.UniBranches.objects.filter(
                domain=data["domain"], course=data["course"]
            )
...
class ALL_SEM_SUBS(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            if data["sub_id"] == "CPC":
                if data["domain"] == "All":
                    all_sub_names = api_models.BranchSub.objects.filter(
                        sub_id=data["sub_id"], domain="@nitc.ac.in"
                    )
                else:
                    all_sub_names = api_models.BranchSub.objects.filter(
                        sub_id=data["sub_id"], domain=data["domain"]
                    )
            else:
                all_sub_names = api_models.BranchSub.objects.filter(
                    sub_id=data["sub_id"], domain=data["domain"], course=data["course"]
                )
...
class ALL_SUB_YEAR_FILES(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            qns_file.qn_ans_file = ContentFile(
                base64.b64decode(data["file"]), data["file_name"]
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                ...
                rating = api_models.Ratings.objects.get(
                    username=user, sub_name=sub_name
                )
                sub_name.tot_ratings_val = (
                    sub_name.tot_ratings_val - rating.rating + data["rating"]
                )
                sub_name.save()
...
class SECURITY(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            bulk_notifications(
                [user1.token],
                user.username + " Reported on " + data["report_belongs"],
                data["description"],
            )
...
class Notifications(APIView):
    ...
    def delete(self, request):
        ...
        try:
            data = request.query_params
            notification = models.Notifications.objects.get(
...
class DATING_USER(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            for i in data:
                try:
                    dating_user_reaction = models.DatingUserReactions.objects.get(
                        username=user, DatingUser=i
                    )
    ...
    def post(self, request):
        ...
        try:
            ...
            datingUser.dummyProfile = ContentFile(
                base64.b64decode(data["file"]), data["file_name"]
            )
            ...
            datingUser.algoValue = (
                datetime_weight * int(datingUser.posted_date.timestamp())
                + like_weight
                * (datingUser.Reactions1_count + datingUser.Reactions2_count)
    ...
    def patch(self, request):
        ...
        try:
            data = request.data
            user = request.user
            datingUser = models.DatingUser.objects.get(
                username=User.objects.get(email=data["dating_user_email"])
            )
    ...
    def put(self, request):
        ...
        try:
            ...
            usersProfiles = []
            for i in datedUuids:
                usersProfiles.append(
                    models.DatingUser.objects.get(
                        username=User.objects.get(user_uuid=i)
                    )
                )
...
class DATING_USER_REACTIONS(APIView):
    ...
    def post(self, request):
        ...
        try:
            data = request.data
            user = request.user
            dating_user = models.DatingUser.objects.get(
                username=User.objects.get(email=data["dating_user_email"])
            )
            try:
                dating_user_reaction = models.DatingUserReactions.objects.get(
                    username=user, DatingUser=dating_user
                )
                if data["Reaction"] == 1:
            ...
            dating_user.algoValue = (
                datetime_weight * int(dating_user.posted_date.timestamp())
                + like_weight
                * (dating_user.Reactions1_count + dating_user.Reactions2_count)
